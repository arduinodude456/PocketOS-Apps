...
// Standalone PocketLua SD-app runtime.
PocketLua pocketLua;
String pocketLuaSource = "";
bool pocketLuaLoaded = false;
uint32_t pocketLuaTimerInterval = 0;
uint32_t pocketLuaLastTimer = 0;
...
void appVmExecuteEvent(const String &name);
bool isLuaAddon(const char *path);
void pocketLuaClose();
bool pocketLuaLoad(const char *path);
void pocketLuaRender();
void pocketLuaRunTimers();
void pocketLuaTouch(const String &action);
...
void loadAddonRuntime(const char *path);
...
void executeAddonAction(String action) {
  ...
  if (lower.startsWith("lua:")) {
    pocketLuaTouch(action.substring(5));
  ...
  if (isAddonFile(path)) {
    ...
    for (uint8_t i = 0; i < addonCount; ++i) {
      if (path == addonApps[i].path) {
        activeAddon = i;
        loadAddonRuntime(addonApps[i].path);
...
void handleAppsTap(int x, int y) {
  ...
  appInput = "";
  appRuntimeMessage = "";
  loadAddonRuntime(addonApps[index].path);
  openScreen(SC_APP_RUNNER);
}
...
void updateAnimations() {
  ...
  if (isLuaAddon(addonApps[activeAddon].path)) pocketLuaRunTimers(); else appVmRunTimers();
...
// ---------------------------------------------------------------------------
// PocketOS host bindings for the standalone PocketLua interpreter
// ---------------------------------------------------------------------------
...
static long plRandom(void*, long maximum) { return maximum>0 ? random(maximum) : 0; }
static void plOn(void*, const char*) {}

void pocketLuaClose() { pocketLuaLoaded=false; pocketLuaSource=""; pocketLuaTimerInterval=0; }
bool isLuaAddon(const char *path) { return lowerCopy(path).endsWith(".lua"); }
void pocketLuaHostSetup() { PocketLuaHost host={}; host.context=nullptr; host.label=plLabel; host.value=plValue; host.grid=plGrid; host.cell=plCell; host.button=plButton; host.message=plMessage; host.timer=plTimer; host.random=plRandom; host.on=plOn; pocketLua.setHost(host); }
...
void drawAddonRunner(){if(activeAddon>=0&&isLuaAddon(addonApps[activeAddon].path))pocketLuaRender();else appVmRender();}
...
{
  String value = lowerCopy(path);
  return value.endsWith(".papp") || value.endsWith(".app") || value.endsWith(".lua");
}
